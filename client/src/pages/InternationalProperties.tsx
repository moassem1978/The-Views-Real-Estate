...
export default function InternationalProperties() {
  const [location] = useLocation();
  const [filters, setFilters] = useState<SearchFilters>({ international: true });
  
  // Extract query parameters from URL, but always maintain international=true
  useEffect(() => {
    const params = new URLSearchParams(location.split('?')[1]);
    
    const newFilters: SearchFilters = { international: true };
  ...
  const apiUrl = `/api/properties/search?${queryString}`;
  ...
  const handleFilterChange = (newFilters: SearchFilters) => {
    // Always ensure international filter is true
    const updatedFilters = { ...newFilters, international: true };
    setFilters(updatedFilters);
    
    // Update URL with new filters
    const params = new URLSearchParams();
    Object.entries(updatedFilters).forEach(([key, value]) => {
      if (value !== undefined && value !== null && key !== 'international') {
        params.append(key, String(value));
      }
    });
    
    const newQueryString = params.toString();
    const newPath = newQueryString ? `/international?${newQueryString}` : '/international';
        ...
        <main className="flex-grow">
          <div className="container mx-auto px-4 py-12 text-center">
            <h1 className="text-3xl font-serif font-semibold text-gray-800 mb-4">
              Error Loading International Properties
            </h1>
        ...
        <section className="bg-[#333333] py-16 relative">
          ...
          <div className="container mx-auto px-4 relative z-10 text-center">
            ...
            <p className="text-lg text-white/90 max-w-2xl mx-auto">
              Explore luxury real estate opportunities beyond Egypt. Discover premium properties in sought-after global destinations.
            </p>
          </div>
        </section>
